...
- [Persistent Container Mode](#persistent-container-mode)
...
Codex CLI can be run in a persistent container mode that provides an HTTP API for starting and managing jobs.
...
OPENAI_API_KEY=your_api_key ./codex-cli/scripts/run_persistent_container.sh

# Using the CLI directly
codex --server --port 3000
```

### API Endpoints

- `POST /api/jobs` - Start a new job
  - Request body: `{ "prompt": "your prompt", "model": "optional-model", "approvalPolicy": "optional-policy" }`
  - Response: `{ "jobId": "123", "status": "running" }`

- `GET /api/jobs/:jobId` - Get job status and logs
  - Response: `{ "jobId": "123", "status": "running|completed|failed", "logs": [...] }`

- `GET /api/jobs` - List all jobs
  - Response: `{ "jobs": [{ "jobId": "123", "status": "running", "createdAt": "..." }, ...] }`

- `POST /api/jobs/:jobId/cancel` - Cancel a running job
  - Response: `{ "jobId": "123", "status": "cancelled" }`
